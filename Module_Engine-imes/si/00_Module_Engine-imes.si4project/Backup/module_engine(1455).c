...
static char* s_report_status = NULL;	//用于缓存状态位上报的hex，大小为payload长度，用以缓存status数据，作增量上报
...
COMMON_API int json_to_hex(const char* json_data, OUT char* uart_hex)
{
	...
	CHECK_POINT(req);
	//固定的
	char* hex = F_MALLOC(s_command_len + 4); 	//4字节包含head和len, 完整的电控板命令行
	CHECK_POINT(hex);
	...
	//unsigned char crc_res = get_crc(hex, pos);

	unsigned char crc_res = Crc16(hex, 0, pos);
