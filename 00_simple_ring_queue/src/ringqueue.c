...
static int ring_queue_write_error_check(ring_queue* ring, uchar* buf, uint len)
{
	if(NULL == ring) 
	{
		MPrint("Ring_Error");
...
int write_queue_ring_op(ring_queue* ring, uchar* buf, uint len)		//写入队列数据
{
	int res = ring_queue_write_error_check(ring, buf, len);
...
static int ring_queue_read_error_check(ring_queue* ring, uchar* buf, uint len)
{
	if(NULL == ring) 
	{
		MPrint("Ring_Error");
...
int read_queue_ring_op(ring_queue* ring, uchar* buf, uint len)		//读出队列数据
{
	int res = ring_queue_read_error_check(ring, buf, len);
...
int read_all_queue_ring_op(ring_queue* ring)			//读出现有队列所有数据
{
	if(NULL == ring) 
	{
		MPrint("RING_ERROR");
